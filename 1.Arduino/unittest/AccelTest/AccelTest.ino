...
void setup() {
  ...
  // if (Serial) {
  //   Serial.println("[STATUS] Serial communication has initiated.");
  //   if (Serial2) {
  //     Serial.println("[STATUS] Zigbee communication has initiated.");
  //     Serial2.print("[STATUS] Zigbee communication has initiated.\r\n");
  //   } else {
  //     Serial.println("[ ERROR] Fail to initiate communication via Zigbee.");
  //   }
  // }
  ...
  if (curr_time - prev_time >= PERIOD_DATA_RETRIEVAL) {
    ...
    if (rocket_on) {
      if (pres_altitude >= max_altitude) {
        ...
        if (max_altitude - pres_altitude >= DROGUE_PARA_THRESHOLD) {
        ...
        if (max_altitude - pres_altitude >= MAIN_PARA_THRESHOLD) {
    ...
    data[datanum].data_packet.on = rocket_on;
  ...
  if (datanum == MAX_DATA_NUM - 1) {
    ...
    // if (file_sd) {
      ...
      if (save_cycle >= SAVE_CYCLE) {
        ...
        Serial.print(millis());
        Serial.println("ms.");
        Serial2.print("[STATUS] Data saved at ");
    ...
    //   if (!SD.begin(53)) {
    //     // SD card is lost
    //     Serial.println("[STATUS] Device error: SD card.");
    //     Serial2.print("[STATUS] Device error: SD card.\r\n");
    //   } else {
    //     // We found the SD card!
    //     Serial.println("[STATUS] SDcard found! Recollecting data ...");
    //     Serial2.print("[STATUS] SDcard found! Recollecting data ...\r\n");

    //     // Open a new file
    //     memset(file_name, '\0', sizeof(char) * 50);
    //     sprintf(file_name, "%s%u.txt", FILE_NAME, millis());
    //     file_sd = SD.open(file_name, FILE_WRITE);
    //     Serial.print("[STATUS] File opened: ");
    //     Serial.println(file_name);
    //     Serial2.print("[STATUS] File opened: ");
